	...
	namespace Core
	{
		...
		public:
			...
			bool try_reconnect(int reconnect_number = 20)
			{
				std::stringstream ss{};
				bool success = false;
				while (reconnect_number != 0 && !success)
				{
					ss.str("");
		...
		private:
			...
			std::unordered_map<QVPNSocketData, Socket> response_sockets_;
		...
		private:
			...
			decltype(auto) add_to_response_socket_map(Socket& response_socket, QVPNSocketData& remote_key)
			{
				QVPNSocketData sock_data(remote_key);
				QVPNSocketSettings settings(true, 3000);
				bool legal_addr = false;
				response_socket.apply_settings(settings);
				while (!legal_addr)
				{
					...
					{
						response_sockets_[sock_data] = response_socket;
						legal_addr = true;
					}
					else
						legal_addr = true;
			...
			std::optional<Socket> connect_to_server_impl_(Socket& response_socket, QVPNSocketData& key, const QTunnelProxyData& proxy_data)
			{
				...
				add_to_response_socket_map(response_socket, key);
			...
			std::variant<NoNetTcpPacket, NoNetUdpPacket> generate_packet_with_transport_layer(Socket& socket, const QTunnelProxyData& data, std::variant<TCPPacketObject, UDPPacketObject>& transport, UByte* begin, UByte* end)
			{
				auto t_proto = data.get_transport_proto();
			...
			std::variant<FullTcpPacket, FullUdpPacket> generate_sender_packet(const QTunnelProxyData& data, UByte* begin, UByte* end)
			{
			...
			void listen_response_sockets_()
			{
				while (true)
				{
					for (auto& [key, sock] : response_sockets_)
					{
						auto rec_data = sock.recv_from(key.local_addr, key.local_port);
						if (rec_data.status.success)
							vpn_response_loop_iteration<>(sock, key, rec_data.data.data(), rec_data.data.data() + rec_data.size);
						else
						{
							std::stringstream ss{};
