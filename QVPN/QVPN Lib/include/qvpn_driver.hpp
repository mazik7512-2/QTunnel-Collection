...
namespace QVPN
{
	namespace Core
	{
		...
		private:
			...
			mutable UByte last_packet_id_ = 0; // при переполнении должен обнул€тьс€
		...
		public:
			...
			{
				// при декодировании нужен обратный пор€док
				std::vector<BaseTypes::UByte> res_data(begin, end);
		...
		public:
			...
			QVPNConnectionElement(const Ipv4AddressType& address, BaseTypes::UShort port, TransportProtocol transport_proto = TRANSPORT_UNDEFINED)
				: addr_(address), port_(port), t_proto_(transport_proto)
			{

			}

			QVPNConnectionElement(const Ipv6AddressType& address, BaseTypes::UShort port, TransportProtocol transport_proto = TRANSPORT_UNDEFINED)
				: addr_(address), port_(port), t_proto_(transport_proto)
			{
			}

			QVPNConnectionElement(std::string_view data, BaseTypes::UShort port, TransportProtocol transport_proto = TRANSPORT_UNDEFINED)
				: addr_(data), port_(port), t_proto_(transport_proto)
			{

			}

			QVPNConnectionElement(const AddrType& address, BaseTypes::UShort port, TransportProtocol transport_proto = TRANSPORT_UNDEFINED)
				: addr_(address), port_(port), t_proto_(transport_proto)
			{
		...
		public:
			...
			QVPNConnectionSettings(const Ipv4AddressType& address, BaseTypes::UShort port, TransportProtocol t_proto)
				: data_(address, port, t_proto)
			{

			}

			QVPNConnectionSettings(const Ipv6AddressType& address, BaseTypes::UShort port, TransportProtocol t_proto)
				: data_(address, port, t_proto)
			{
			}
		...
		public:
			...
			bool reconnect()
			{
				...
				if (res.success) {
					...
					ss << "Reconnection to QVPN Server (" << socket_.get_remote_addr().to_string() << ":" << socket_.get_remote_port() << ") failed";
					logger_.fail(ss.str());
				}
				return res.success;
			}
		...
		public:
			...
			void add_addr(const Ipv4AddressType& addr, UShort port, TransportProtocol t_proto);
			void add_addr(const Ipv6AddressType& addr, UShort port, TransportProtocol t_proto);
			void add_addr(const NetAddr& addr, UShort port, TransportProtocol t_proto);
		...
		private:
			...
			NetProtocol net_proto_;
			TransportProtocol transport_proto_;
		...
		private:
			...
			{
				auto t_proto = socket.get_transport_protocol();
				...
				case TransportProtocol::TCP:
				{
					...
					TCPPacketObject obj = TCPPacketObject::generate_object(data.get_src_port(), data.get_dst_port(), scheme.get_seq(), scheme.get_ack(), scheme.get_offset(), scheme.get_flags(), scheme.get_window(), scheme.get_urgent_pointer());
					return obj;
					break;
				}
			...
			std::variant<NoNetTcpPacket, NoNetUdpPacket> generate_packet_with_transport_layer(Socket& socket, std::variant<TCPPacketObject, UDPPacketObject>& transport, UByte* begin, UByte* end)
			{
				auto t_proto = socket.get_transport_protocol();
			...
			bool vpn_loop_iteration(std::shared_ptr<Socket> client_socket, std::unordered_map<QVPNSocketData, Socket>& sock_map, Stats& stats, std::string_view user)
			{
				...
				auto packet = generate_packet_with_transport_layer(server_socket, t_header, b, e);
				auto packet_data_size = std::distance(b, e);
				...
				auto [res_b, res_e] = std::visit([&server_socket, &packet_data_size](auto& p)
					{
						...
						p.recalculate_checksums(src, dst, packet_data_size);
						return p.bytes(); 
					}, packet);
			...
			void listen_and_connect_socket_(Socket& socket, Database& database, Stats& stats)
			{
				...
				// TODO: сделать логер отдельными функци€ми
